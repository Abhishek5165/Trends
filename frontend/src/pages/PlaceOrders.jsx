...
const PlaceOrders = () => {
  ...
  const { nevigate, backendURL, token, cartItems, setCartItems, getCartAmount, delivery_fee, products } = useContext(ShopContext);
      ...
      handler: async (response) => {
        ...
        try {
          const verifyData = {
            ...response,
            amount: order.amount,
            address: order.address || formData,
            items: order.items || []  // Ensure items are passed
          };
      ...
      switch (method) {
        ...
        case 'razorpay':
          let razorpayOrderData = {
            ...orderData,
            amount: orderData.amount * 100  // Multiply by 100 only for Razorpay
          };
          const responseRazorpay = await axios.post(backendURL + '/api/order/razorpay', razorpayOrderData, { headers: { token } });
          if (responseRazorpay.data.success) {
            initPay({ ...responseRazorpay.data.order, amount: orderData.amount, items: orderItems });
          } else {
